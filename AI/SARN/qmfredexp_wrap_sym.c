...
/* Only symmetric kernels with wrap around.  Kernel symmetries are used. */
...
FILTER lf,hf;
{
   ...
   /* horizontal filtering */
   {
     ...
     hor_reflect(in[0],MAX(ltaps,htaps),6);
   ...
   ver_reflect(lo,MAX(ltaps,htaps),6);
   ver_reflect(hi,MAX(ltaps,htaps),6);
...
FILTER lf,hf;
{
   ...
   /* vertical filtering */
   {
     ...
     l = lo.ptr;  h = hi.ptr;
     ver_reflect(in[0],MAX(ltaps,htaps),6);	/* Reflection */
     ver_reflect(in[1],MAX(ltaps,htaps),6);
     ver_reflect(in[2],MAX(ltaps,htaps),6);
     ver_reflect(in[3],MAX(ltaps,htaps),6);
   ...
   /* horizontal filtering */
   {
     ...
     hor_reflect(lo,MAX(ltaps,htaps),6);	/* Reflection */
     hor_reflect(hi,MAX(ltaps,htaps),6);
...
FILTER lf,hf;
{
   ...
   /* vertical filtering */
   {
     ...
     for (n=1; n <= taps; n++) {	/* Reflection */
       for (i=left; i <= right; i++) {
         l[bottom+n-1][i] += l[top+n][i];
         l[top-n+1][i] += l[bottom-n][i];
         h[bottom+n-1][i] += h[top+n][i];
         h[top-n+1][i] += h[bottom-n][i];
       }
     }
   }
   ...
   /* horizontal filtering */
   {
     ...
     for (j=0; j < out[0].h; j++) {
       ...
       for (n=1; n <= taps; n++) {	/* Reflection */
         a[j][left+n-1] += a[j][right+n];
         a[j][right-n+1] += a[j][left-n];
       }
     }
   }
