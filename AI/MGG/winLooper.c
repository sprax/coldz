...
static int	WIDE = 1280;		/* should test in init_GL_config */
static int	TALL = 1024;
static char	Ss[SLEN];
...
static float	newX, newY, oldX, oldY;
    ...
    while(getbutton(LEFTMOUSE)) {
      newX = (getvaluator(MOUSEX) - orgX);
      newY = (getvaluator(MOUSEY) - orgY);
...
#define     MIN_BOX_LEN  100
  ...
  wA->next = showSub(wA,   xb/zX,yb/zY,xn/zX,yn/zY,"subA",zX*izoom,zY*izoom,5);
...
Wnew (pDmj D, char *winTitle, float zX, float zY, unt flags)
{
  ...
  W->zX = zX;
  W->zY = zY;
  zW = D->w*W->zX + 0.5, zH = D->h*W->zY + 0.5;
...
*/
  ...
  for (dp = P, sp = S->P + yi*sw + xi, j = Dm.h; j-- ; sp += (sw - Dm.w))
    for (k = Dm.w; k-- ; )
      *dp++ = *sp++;
  W = showPm(P,&Dm,winTitle,zoomX,zoomY,flags);
#endif
return  W;
}
...
long	dmode;
  ...
  if (W->P) {
    ...
    lrectwrite(0,0,W->D.w-1,W->D.h-1,W->P);
...
IMAGE   *sgim;
Screencoord wd = dW*W->zX + 0.5;
Screencoord ht = dH*W->zY + 0.5;
  ...
  if (flags || !rectP) {
    ...
    if ((nread = lrectread(0,0,wd-1,ht-1,rectP)) != sz)
