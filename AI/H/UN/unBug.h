...
extern void (*death_trap)(void); 		/* extern GLOBAL */
extern void unDumpCore (void);	 		/* extern GLOBAL */
extern void segFault(void);	 		/* extern GLOBAL */
int	sys (const char *fmt, ...); 		/* fprintf(stderr,...) & system(...) */
void	die (const char *fmt, ...); 		/* fprintf(stderr,...) & exit(-1) */
void	prn (const char *fmt, ...); 		/* fprintf(stderr,...), no exit */
void	wrd (const char *fmt, ...); 		/* void function stub: return; */
void	wrn (const char *fmt, ...); 		/* stderr:"Warning -- ...", no exit */
void	warn(const char *fmt, ...); 		/* stderr:"Warning -- ...", no exit */
...
#define DIE(FUN,MSG)	die("%s [%s]: %s",FUN,__FILE__,MSG);
...
#else
#define  COREDUMP       {char *_c_d_ = (char *)0; *_c_d_ = 'c';}
#endif
