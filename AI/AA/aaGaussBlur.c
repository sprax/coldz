...
puc GaussianBlur (dst,      src,      sdwide, sdtall,xlen,ylen,nChan,kind,flags)
             puc  dst; pcuc src; int *sdwide,*sdtall,xlen,ylen,nChan,kind,flags;
{   
  if (0 < xlen && xlen < MAX_BLUR_LENGTH &&
      0 < ylen && ylen < MAX_BLUR_LENGTH) {
      return( ConvoSep(dst,src,GaussiansN[xlen],GaussiansN[ylen]
                      ,sdwide,sdtall,xlen,ylen,nChan,kind,flags));
      }
  fprintf(stderr,"GaussianBlur: bad x or y blur length\n");
  return(NULL);
}


puc RadialBlur (dst,      src,      sdwide, sdtall,radius,nChan,kind,flags)
           puc  dst; pcuc src; int *sdwide,*sdtall,radius,nChan,kind,flags;
{ 
    int q = 2*radius + 1;
    if (0 < q && q < MAX_BLUR_LENGTH)
        return( ConvoSep(dst,src,GaussiansN[q],GaussiansN[q]
                        ,sdwide,sdtall,q,q,nChan,kind,flags));
    fprintf(stderr,"RadialBlur: bad blur radius\n");
    return(NULL);
}


puc CraigBlur (dst,      src,      sdwide, sdtall,radius,nChan)
...
{ 
    ...
    fprintf(stderr,"CraigBlur: bad blur radius\n");
    return(NULL);
}
...
/*___________________________________________________________________________*
    ConvoSep: Functionally the same as PxConvoSep (in aaPxConvo.c), but without
...
puc ConvoSep (dst,      src,            xmsk, ymsk,      sdwide, sdtall,mwide,mtall,nChan,kind,flags)
         puc  dst; pcuc src; const int *xmsk,*ymsk; int *sdwide,*sdtall,mwide,mtall,nChan,kind,flags;
{
    ...
    *sdtall = ttall-mtall+1;
    if (DEBUG > 2)
        printf("ConvoSep: %d %d --convo--> %d %d\n", twide,ttall,*sdwide,*sdtall);
    
    /*_________________Do the convolution___________________________*/
    switch (kind)  {
        case GRAY:   dst = ConvoSepGRAY(  dst,tmp,xmsk,ymsk,twide,ttall,mwide,mtall); break;
        case RGBI:   dst = ConvoSepRGBI(  dst,tmp,xmsk,ymsk,twide,ttall,mwide,mtall); break;
        case RGBP:   dst = ConvoSepRGBP(  dst,tmp,xmsk,ymsk,twide,ttall,mwide,mtall); break;
        case RGBTI:  dst = ConvoSepRGBTI( dst,tmp,xmsk,ymsk,twide,ttall,mwide,mtall); break;
        case RGBTCI: dst = ConvoSepRGBTCI(dst,tmp,xmsk,ymsk,twide,ttall,mwide,mtall); break;
        default: fprintf(stderr,"ConvoSep: kind unknown: %d\n",kind);  return(NULL);
    }
    if (! (flags & CONTRACT))
        free(tmp);
    if (dst == NULL)
        fprintf(stderr,"ConvoSep: convolution failed\n");
    ENDCLOCK("ConvoSep");
    return(dst);
}

/*___________________________________________________________________________*
    ConvoSepf (in aaGaussianBlurf.c):
    Functionally the same as PxConvoSepf (in aaPxConvoF.c), but without
    the Px structs.  The source is (optionally) padded before being convolved
