/*****************************************************************************
 p2sgi.c -- Sarnoff img to SGI rgb    S.R.Lines (< Paul Haeberli)  93.5.24
 *****************************************************************************/
...
char **argv;
{
    ...
    int y, depth, maplen, sarntype;
    unsigned char *fb, *fbptr;
    int fblength;
    sarnimg_header hdr;

    if (argc<3) {
	fprintf(stderr,"usage: p2sgi sarnimage irisimage\n");
	exit(1);
    }
    ...
    if (Options & VERBOSE)
    switch (sarntype) {
      case SARN_UBYTE: fprintf(stderr,"\tSarnType: SARN_UBYTE, ok!\n");  break;
      case SARN_BYTE:  fprintf(stderr,"\tSarnType: SARN_BYTE, ok!\n");   break;
    ...
    itype = image->type; 
    fprintf(stderr,"image->type: %d, ISVERBATIM: %d, ISRLE: %d, BPP: %d\n"
	...
	case 8:
	    ...
	    Yes, the following is a waste of disk space:
	    ***********************************/
	    putrowch(image,cbuf,(ysize-1)-y,0);
	    putrowch(image,cbuf,(ysize-1)-y,1);
	    putrowch(image,cbuf,(ysize-1)-y,2);
	    break;
	case 24: 
	    fread(cbuf,rowbytes,1,inf);
