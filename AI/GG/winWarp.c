...
#define outCn(frp,x,y,w,h,dir,name,num,tot)     \
        sprintf_s(path,"%s/%s.%0*d",dir,name,DECIMALS(tot),num);\
...
int showNewFrameMain(pMD wwD)
{
    ...
    if (Devopts & O_WRIT) {
        sprintf_s(Ss, SLEN, "%s/v.%0*d", ArgStr[NumSrc + 2], 3, nframes);
...
newFrameFromClrs(flt clrs[], int numSrc)
{
    ...
    sprintf_s(Ss, SLEN, "%s frame %d, time %.3f", winTitle, nframes + 1, cpuT);
    wintitle(Ss);
    ...
    if (strcmp(str2, "No")) {
        ...
        if (nbytes > 26) {
            ...
            if (err) {
                ...
                fclose(fi);  fi = NULL;
                sprintf_s(Ss, SLEN, "rm -f %s", ipath);
                system(Ss);
            ...
            case 205: if (!fvX[nB] || !wwD->bvX[nB]) {
                ...
                warn("reFlowGrid requires both forward and backward flows."); break;
                      ...
                      RETITLE("reFlowGrid A<->B"); reFlowGridWarp(v, 0, 0);  if (Devopts & O_EXTR) {
                          RETITLE("ReGrid+Save"); writGridTet(v, GrdY, GrdX, 0); saveFlowFiles(wwD, nA, nB);
                      } break;
