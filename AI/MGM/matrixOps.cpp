/*****************************************************************************
matrixOps.c -- from Kah-Kay Sung, NRC, etc.
 *****************************************************************************/
...
int sludcmp(float **a, int n, int *indx, float *d);
...
{
  float **b;
printf("duh, invert_matrix"); exit(0);	
  return (long) gaussj(a, n, b, 0);
}
...
{
  float *res;
  if ((res = (float *)malloc(sizeof(float) * (n+1))) == (float *) '\0') {
...
static void compute_ata_and_atb(float **a, float **b, long m, long n, float **ata, float **atb)
...
int sludcmp(float **a, long n, long *indx, float *d)
{
  ...
  *d=1.0;
  for (i=1;i<=n;i++) {
    ...
    if (big == 0.0) {
      fprintf(stderr, "sludcmp():  Singular matrix in routine LUDCMP");
      fflush(stderr);
      free_vector(vv);
      return -1;
    };
