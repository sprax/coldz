...
{
  while(dD--)
    rowBordFltBrd(img[dD],dL,dT,dW,dH,cL,cT,cW,cH,sL,sT,sW,sH,rtype);
}

void
rowBordFltBrd ( ppf img, int dL, int dT, unt dW, unt dH
                       , int cL, int cT, unt cW, unt cH
...
ppf	vM;	
 ...
 switch(rtype) {
   ...
   case  1:				/* odd reflection: edge skipped */ 
     ...
     bL = dL-cL;     bT = (bL-cL)-(sW-sL);  if (bT > 0)  bL -= bT;
     bR = dW-dL-cW;  bB = bR-mW-sL;         if (bB > 0)  bR -= bB;
     for (vM = img-cT, j = 0; j < cH; j++ ) {
