...
readImgLabels (char *pathname)
{
  ...
  if (!(fp = fopen (pathname, "r"))) {
    /*
    warn("readImgLabels(%s) [%s]: err fopen, returning NULL",pathname,__FILE__);
    */
    return NULL;
  }
  while (fgets (buf, 256, fp) != NULL) {
    if (buf[0] == '#')  continue;
    if (sscanf (buf, "%s %f %f", name, &x, &y) == 3) {
      img_label = (img_label_t *) calloc (1, sizeof (img_label_t));
      img_label->name = strdup (name);
      img_label->x = x;
      img_label->y = y;
...
dupImgLabels (List src)
{
  ...
  for (dst = NULL; src != NULL; src = src->cdr) {
    ...
    src_label = src->car;
    dst_label->name = strdup(src_label->name);
