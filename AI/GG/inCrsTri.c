...
zoomEditMenu2(void *v, int device, int flags)	/* diff. menu for zoomed windows? */
{
    ...
    pMD     wwD = (pMD)mn->data;
    long 	pval;
#define retitle(S)      sprintf_s(str,"%s: %s",wA->T,(S)); wintitle(str)
...
zoomRectTri2(void *v, int device, int flags)
{
    ...
    while (getbutton(device)) {
        ...
        if (xAn != xAo || yAn != yAo) {
            ...
            rects((Scoord)xAb, (Scoord)yAb, (Scoord)xAo, (Scoord)yAo);
...
#define MIN_BOX_LEN  20
    ...
    int yi = (int)(xAn / zX);
    int yf = (int)(yAn / zY);
    wp->next = showSub(wA->next, xi, xf, yi, yf, "subB", mB->nzX, mB->nzY, 7);
    wp->next->in = nB;
    wp->next->act = &moveTri;
    wp->next->mnu = &zoomMnu;
    wp->next->next = wp;
