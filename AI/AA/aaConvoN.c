/*******  aaConvoN.c
          ...
          arguments may include positive and negative integers; no one
          way of normalization is appropriate to for all operations this
          makes possible, and so no normalization, rectification,
          ...
          Also, this code is "mature" and contains no bugs!   *:)   But just
          in case:
#define   DEBUG 4
#include "aaBugsy.h"
********/
...
/*___________________________________________________________________________*
    ConvoNClipSepGRAY/RGBI/RGBTI: use for 2-dimensional convolution when the filter
    ...
    Called by PxConvoNClipSep.
 *___________________________________________________________________________*/

puc ConvoNClipSepGRAY (dst,      src,            xmsk, ymsk,     swide,stall,mwide,mtall)
                 puc  dst; pcuc src; const int *xmsk,*ymsk; int swide,stall,mwide,mtall;
{
    ...
    BEGCLOCK("ConvoNClipSepGRAY");  /* #ifdef TIMING, then clock this function (aa.h).*/
    
    if (! (tmp = (puc) malloc((size_t)dwide*stall)) || dst == src ||
        ! (dst = (puc)realloc(dst,(size_t)dwide*dtall))) {
        fprintf(stderr,"ConvoNClipSepRGBI: bad malloc or src/dst pointers\n");
    ...
    free(tmp);
    ENDCLOCK("ConvoNClipSepGRAY");  /* #ifdef TIMING, finish what BEGCLOCK started. */
    return(dst);
}              
...
puc ConvoNClipSepRGBI (dst,      src,            xmsk, ymsk,     swide,stall,mwide,mtall)
             puc  dst; pcuc src; const int *xmsk,*ymsk; int swide,stall,mwide,mtall;
{
    register int const *mp;   /* Register vars declared in order of frequency. */
    register pcuc sp;
    ...
    BEGCLOCK("ConvoNClipSepRGBI");  /* #ifdef TIMING, then clock this function (aa.h).*/
    if (! (tmp = (puc) malloc((size_t)dwide*stall)) || dst == src ||
        ! (dst = (puc)realloc(dst,(size_t)dwide*dtall))) {
        fprintf(stderr,"ConvoNClipSepRGBI: bad malloc or src/dst pointers\n");
        return(NULL);  /* Clean up or exit is left to the caller. */
    ...
    free(tmp);
    ENDCLOCK("ConvoNClipSepRGBI");
    return(dst);
}              

/*________________RGB in BANKS version____________________________________________*/

puc ConvoNClipSepRGBP (dst,      src,            xmsk, ymsk,     swide,stall,mwide,mtall)
             puc  dst; pcuc src; const int *xmsk,*ymsk; int swide,stall,mwide,mtall;
{
    register int mval;
    register int const *mp;   /* Register vars declared in order of frequency. */
    ...
    BEGCLOCK("ConvoNClipSepRGBP");  /* #ifdef TIMING, then clock this function (aa.h).*/
    if (! (tmp = (puc) malloc(tsize*NCHAN)) || dst == src ||
        ! (dst = (puc)realloc(dst,(size_t)dwide*dtall*NCHAN))) {
        fprintf(stderr,"ConvoNClipSepRGBP: bad malloc or src/dst pointers\n");
        return(NULL);  /* Clean up or exit is left to the caller. */
    }  
...
puc ConvoNClipSepRGBTI (dst,      src,            xmsk, ymsk,     swide,stall,mwide,mtall)
              puc  dst; pcuc src; const int *xmsk,*ymsk; int swide,stall,mwide,mtall;
{   /* 4 Channels Interleaved: R,G,B,T = Transparency. */
    ...
    free(tmp);
    ENDCLOCK("ConvoNClipSepRGBTI");
    return(dst);
}              

/*__________RGBTCI: FOR CONSTANT TRANSPARENCY/ALPHA CHANNEL ONLY!!!!____________________*/

puc ConvoNClipSepRGBTCI (dst,      src,            xmsk, ymsk,     swide,stall,mwide,mtall)
               puc  dst; pcuc src; const int *xmsk,*ymsk; int swide,stall,mwide,mtall;
