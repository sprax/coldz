...
{
  while(dD--)
    rowBordUnc(img[dD],dL,dT,dW,dH,cL,cT,cW,cH,sL,sT,sW,sH,rtype);
}
...
/*******
     wrn("rowBordUnc: %3d %3d %3u %3u"   ,dL,dT,dW,dH);
     wrn("          , %3d %3d %3u %3u"   ,cL,cT,cW,cH);
     wrn("          , %3d %3d %3u %3u %d",sL,sT,sW,sH,rtype);
 *******/

     bL = dL-cL;  bT = cW+cL;  if (bL > bT)  bL = bT;

     bR = dW-dL-cW;  bB = bR-mW-sL;         if (bB > 0)  bR -= bB;
     for (vM = img-cT, j = 0; j <= mH; j++ ) {
   ...
   case  8:				/* weird reflection */
     ...
     break;
#endif
    default: die("rowBordUnc: NOIMP for case %d",rtype);
  }
}
