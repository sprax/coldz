...
lsq_opt(image1,image2,vx,vy)	/* Courtesy of Haim */
FIMAGE image1,image2,vx,vy;
{
...
lsq_fast(image1,image2,vx,vy)	/* Courtesy of Haim */
FIMAGE image1,image2,vx,vy;
{
   register float **img1=image1.ptr,**img2=image2.ptr;
   float **dx=vx.ptr,**dy=vy.ptr,S,C,A,B;
...
lsq_fourier(image1,image2,vx,vy)	/* Courtesy of Haim */
FIMAGE image1,image2,vx,vy;
{
...
lsq_pr(image1,image2,velx,vely)
FIMAGE image1,image2,velx,vely;
{
   register float **img1=image1.ptr,**img2=image2.ptr;
   float **dx=velx.ptr,**dy=vely.ptr;
   float vx,vy,delvx,delvy,epsilon,ldif,edif,dfd,get_dfd();
...
float get_dfd(image1,image2,i,j,vx,vy)
...
printq()	/* Write Q matrix to stdout */
{
   int i,j,k;
...
lsq_grad(image1,image2,vx,vy)
FIMAGE image1,image2,vx,vy;
{
   float **img1=image1.ptr,**img2=image2.ptr,**dx=vx.ptr,**dy=vy.ptr;
   float Fx,Fy,Ft,Fxl,Fyl,Ftl,Fxr,Fyr,Ftr,Fxx,Fxy,Fyy,Fxt,Fyt, conf;
...
row_grad_sum(image1,image2,row,fxx,fxy,fyy,fxt,fyt)
FIMAGE image1,image2;
int row;
...
lsq_lsqgrad(image1,image2,vx,vy)
FIMAGE image1,image2,vx,vy;
{
   FIMAGE Gx,Gy,Gt;
   float **gx,**gy,**gt;
   ...
   float **alloc_farray(),*alloc_fvector();
   int left,right;
   int w=image1.w,h=image1.h,x,y,xx,yy,i,j,k,n;
   char file[MAX_STR_LEN];
...
FIMAGE image1,image2,vx,vy,conf;
{
   ...
   float **alloc_farray(),*alloc_fvector(),*_alloc_fvector();
   int row;
   int w=image1.w,h=image1.h,x,y,i,j,n,nrot;
...
void printwgama()	/* Write W matrix and gama to stdout */
{
...
void printeigens()	/* Write eigenvalues and eigenvectors to stdout */
{
...
lsq_lsqgrad_fast_mask(image1,image2,mask,vx,vy,conf)
FIMAGE image1,image2,mask,vx,vy,conf;
{
   FIMAGE Gx,Gy,Gt;
