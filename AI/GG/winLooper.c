...
labelFace(void *v, int device, int nlabs)
{
    ...
    if (nl > nlabs) {
        sprintf_s(Ss, SLEN, "%s.lbl", wA->T);
        errno_t err = fopen_s(&ofp, Ss, "w");
        if (err)
            die("fopen_s failed on <%s>\n", Ss);
    ...
    while (getbutton((Device)device)) {
        ...
        if (xn != xo || yn != yo) {
            ...
            color(GREEN);
            circi(xn, yn, CRAD);
            sprintf_s(Ss, SLEN, "%s label %ld  xy: %.1f %.1f  (%.7f  %.7f)"
...
pWmj Wclone(pWmj W, int flags)
{
    ...
    sprintf_s(Ss, SLEN, "%s - cloned", W->T);
...
pWmj cloneGrid(pWmj W, int flags)
{
    ...
    if (W->F) {
        ...
        sprintf_s(Ss, SLEN, "%s - cloneGrid", W->T);
...
{ /* Uses lrectread to get W->P mixed with (GL) drawn overlays */
    ...
    if (!W->ti) {
        sprintf_s(path, TLEN, "%s.%03ld", tag, W->id); /* Would set to default icon title...   */
        W->ti = _strdup(path);              /* but can't find equiv. of getwintitle */
    }                                    /* set W->ti so we see where file went. */
    else sprintf_s(path, TLEN, "%s.%s", tag, W->ti);
    sprintf_s(ss, SLEN, "%s: saving: %s ...", W->T, path);
    wintitle(ss);
    ...
    if (j) {
        sprintf_s(ss, SLEN, "%s: ERROR %d saving %s", W->T, j, path);
    }
    else {
        sprintf_s(ss, SLEN, "%s: saved as %s", W->T, path);
    }
    wintitle(ss);
}
...
saveRGB(long wid)
{
    ...
    if (!path) {
        sprintf_s(ss, "RGB.%03d", wid);  /* Would set to default icon title...   */
        W->ti = path = _strdup(ss);	 /* but can's find equiv. of getwintitle */
    }				 /* set W->ti so we see where file went. */
    sprintf_s(ss, "%s: saving %s ...", W->T, path);
    wintitle(ss);
    if (!(sgim = iopen(path, "w", RLE(1), 3, dW, dH, 3))) {
    ...
    j = iclose(sgim);/* 0:OK, -1:fclose error (check errno), EOF:libgutil err */
    if (j) sprintf_s(ss, "%s: ERROR %d saving %s", W->T, j, path);
    else {
#ifdef DEBUG
        sprintf_s(ss, "rectimg %s", path); sys(ss);
#endif
        sprintf_s(ss, "%s: saved as %s", W->T, path);
    }
    wintitle(ss);
    return j;
}
...
long hits;
{
    ...
    for (h = 0; h < hits; h++) {
        ...
        for (i = 0; i < items; i++) {
            if (i != 0)
                charstr(" ");
            sprintf_s(Ss, SLEN, "%d", buffer[indx++]);
            charstr(Ss);
        }
        charstr(") ");
    }
...
#define RETITLE(S)  	sprintf_s(Ss, SLEN, "%s: %s",Wj[iw]->T,(S)); wintitle(Ss)
#define WLmodeTil(M,S)	WLmode = M; RETITLE(S)

    for (;;) {
        if (WLmode == SWINGING && WLswing != NULL)
            while (!qtest())
                WLswing();
        else if (WLmode == LOOPING  && WLstep != NULL)
            while (!qtest())
                WLstep();
        ...
        switch (qdev = (ush)qread(&qval)) {
            ...
            case LEFTMOUSE:		/* qval = 1 for mouseDown, 0 for mouseUp */
                ...
                else if ((wa = Wj[iw]->act))
                    wa->func(Wj[iw], LEFTMOUSE, qval);
                break;
